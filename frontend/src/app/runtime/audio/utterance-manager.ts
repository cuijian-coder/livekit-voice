...
export class UtteranceManager {
  ...
  /**
   * Synchronously adopt a turnId generated by voice-machine.
   * Called from startTurnTransport so utteranceManager and voice-machine
   * share the same turnId without event round-trip.
   */
  startTurnSync(turnId: string): void {
    if (this.isActive) {
      logger.warn('utteranceManager.startTurnSync.ignored.alreadyActive', { turnId: this.turnId })
      return
    }
    this.turnId = turnId
    this.isActive = true
    audioRecorder.resetWorkletSeq()
    pcmPipeline.reset()
    logger.info('utteranceManager.turn.sync', { turnId: this.turnId })
  }
