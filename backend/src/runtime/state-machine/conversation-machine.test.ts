...
describe('conversationMachine', () => {
  ...
  it('should transition from transcribing to listening on START', () => {
    const actor = createActor(conversationMachine as any)
    actor.start()
    actor.send({ type: 'START' })
    actor.send({ type: 'VAD_END' })
  ...
  it('should transition from thinking to listening on START', () => {
    const actor = createActor(conversationMachine as any)
    actor.start()
    actor.send({ type: 'START' })
    actor.send({ type: 'VAD_END' })
  ...
  it('should transition from speaking to listening on START', () => {
    const actor = createActor(conversationMachine as any)
    actor.start()
    actor.send({ type: 'START' })
    actor.send({ type: 'VAD_END' })
